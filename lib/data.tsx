...
export const screens: ScreenData[] = [
  {
    ...
    accent: "acid",
    body: [
      "const fleet = await dispatch.ready()",
      "// 214 vehicles online",
      "42 en route · 9 loading · 2 idle",
      "ETA variance below 4.1% today",
      "reassign(id: 'F-117', to: 'Bay 3')",
      "delay forecast: clearing in 12m",
    ],
  ...
  },
  {
    ...
    accent: "violet",
    body: [
      "const cart = basket.for(user)",
      "// 3 items · $214.00 · free ship",
      "SKU-8812 restocked 14 min ago",
      "variant(xl, 'stone') → available",
      "checkout.session = await pay(...)",
      "conversion: +18% this week",
    ],
  ...
  },
  {
    ...
    accent: "aqua",
    body: [
      "const visit = records.latest('P-3321')",
      "// next appointment · Thu 10:30",
      "consent updated by Dr. Mehta",
      "vitals: stable across last 3 visits",
      "sendReminder(visit, 24h)",
  ...
  },
  {
    ...
    accent: "ember",
    body: [
      "SELECT cohort, mrr FROM growth",
      "// net new MRR this quarter",
      "expansion $18.4k · churn 1.8%",
      "forecast(90d) → $142k MRR",
      "anomaly: signups +34% (paid ad)",
      "export board → slide deck",
    ],
  ...
  },
  {
    ...
    accent: "violet",
    body: [
      "const scan = drone.capture('T-04')",
      "// 1,284 frames · 96% coverage",
      "defect.confidence('crack') → 0.94",
      "segment = thermo.read(frame 881)",
      "submit report → field crew",
      "model retrained on 12 new cases",
    ],
  ...
  },
  {
    ...
    accent: "acid",
    body: [
      "const batch = lab.inventory('B-77')",
      "// 214 vials · expiry 06/2027",
      "protocol P-12 signed · 2 revisions",
      "temp trace: 2.1°C ± 0.3 all day",
      "next audit window: 18 days",
      "archive(batch, 'cold-storage-2')",
    ],
  ...
  },
  {
    ...
    body: [
      "sync(watch, user.id)",
      "// heart rate 61bpm · hrv 54",
      "sleep.score(7h 22m) → 86",
      "trainings planned for today: 1",
      "alert: recovery is high",
      "export week → summary",
    ],
  ...
  },
  {
    ...
    accent: "ember",
    body: [
      "const txn = ledger.authorize(pay)",
      "// $420.00 → vendor #88 · ok",
      "balance $12,840 · budget 68%",
      "insight: subscriptions +$34/mo",
      "freeze(card, 'travel')",
      "receipt → accountant@corp",
    ],
  ...
  },
  {
    ...
    accent: "acid",
    body: [
      "npx noname deploy --prod",
      "✓ build 2.4s · assets 96KB gz",
      "✓ migrations applied (postgres)",
      "✓ health checks green ×3 regions",
      "✓ edge cache warmed in 1.8s",
  ...
  },
  {
    ...
    accent: "violet",
    body: [
      "session.scan('ticket-0931')",
      "// 2 guests · hall B · row 12",
      "checkin.ok → print passes",
      "next show starts in 26 min",
      "staff override requires pin",
      "queue depth: 3 · all clear",
    ],
  ...
  },
  {
    ...
    accent: "ember",
    body: [
      "const loom = mill.machine(12)",
      "// warp tension 180g · nominal",
      "yield 96.2% · weft breaks: 0",
      "predictive maintenance in 9d",
      "batch #2211 → quality hold",
  ...
  },
  {
    ...
    accent: "aqua",
    body: [
      "grid.energy.balance('district-4')",
      "// solar 2.1MW · load 1.8MW",
      "transit.occupancy → 78% peak",
      "water.main.pressure: nominal",
      "incidents.open: 3 · auto-routed",
      "report to city council — monthly",
    ],
